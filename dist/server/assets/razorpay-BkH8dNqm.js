import { a2 as TSS_SERVER_FUNCTION, a3 as createServerFn } from "./server-D9f26qs9.js";
...
const createRazorpayOrder = createServerFn({
  method: "POST"
}).validator((d) => d).handler(createRazorpayOrder_createServerFn_handler, async ({
  data
}) => {
  const keyId = process.env.VITE_RAZORPAY_KEY_ID || void 0 || "";
  const keySecret = process.env.VITE_RAZORPAY_KEY_SECRET || void 0 || "";
  if (!keyId || !keySecret) {
    throw new Error("Razorpay API Key or Secret is missing in server environment variables.");
  }
...
const verifyRazorpayPayment = createServerFn({
  method: "POST"
}).validator((d) => d).handler(verifyRazorpayPayment_createServerFn_handler, async ({
  data
}) => {
  const keySecret = process.env.VITE_RAZORPAY_KEY_SECRET || void 0 || "";
  if (!keySecret) {
    throw new Error("Razorpay API Secret is missing in server environment variables.");
  }
