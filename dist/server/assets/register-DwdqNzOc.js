...
import { a as useNavigate, R as Route, u as useAuth, i as isProfileIncomplete, d as cn, m as motion, b as Link, o as objectType, e as stringType, l as literalType, s as supabase, t as toast } from "./router-DgB0XHQ2.js";
...
import { A as AuthSplit } from "./auth-split-CwkFEouq.js";
import { L as LoaderCircle } from "./loader-circle-C_Krqs5O.js";
import { E as EyeOff, a as Eye } from "./eye-D55DlEux.js";
import { U as Upload } from "./upload-CTB1O8mQ.js";
...
import "./users-round-cmecjFOs.js";
import "./shield-check-rS97jlck.js";
import "./arrow-left-7FMAoHOB.js";
...
function Register() {
  ...
  const mapContainerRef = reactExports.useCallback((node) => {
    ...
    try {
      ...
      console.error("Leaflet registration map init error:", err);
    }
  }, [leafletLoaded]);
    ...
    try {
      ...
      if (data && data.display_name) {
        ...
        if (data.address.postcode) {
          setValue("pincode", data.address.postcode.replace(/\D/g, "").slice(0, 6), {
            shouldValidate: true,
            shouldDirty: true
          });
        }
  ...
  const searchAddress = async (query, pincodeVal) => {
    ...
    try {
      const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(fullQuery)}&format=json&addressdetails=1&limit=1&countrycodes=in`);
      const data = await res.json();
      if (data && data.length > 0) {
        ...
        const displayParts = item.display_name.split(",").map((p) => p.trim()).filter((p) => !/^\d{5,6}$/.test(p));
        setValue("location", displayParts.slice(0, 3).join(", "), {
          shouldValidate: true,
          shouldDirty: true
        });
        if (item.address && item.address.postcode) {
          const cleanPin = item.address.postcode.replace(/\D/g, "").slice(0, 6);
          if (cleanPin.length === 6) {
            setValue("pincode", cleanPin, {
              shouldValidate: true,
              shouldDirty: true
            });
          }
