...
import { c as createLucideIcon, G as useParams, b as Link, s as supabase, t as toast } from "./router-DgB0XHQ2.js";
...
function JobDetails() {
  ...
  const mapContainerRef = reactExports.useCallback((node) => {
    ...
    setIsMounted(true);
    if (!leafletLoaded || !LRef.current || !job) {
      return;
    }
    if (mapRef.current) {
      mapRef.current.remove();
    ...
    if (latVal && lngVal) {
      ...
      fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(job.location)}&format=json&limit=1`).then((res) => res.json()).then((data) => {
        if (data && data.length > 0) {
          const item = data[0];
          initializeMapInstance(parseFloat(item.lat), parseFloat(item.lon));
        } else {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "lg:col-span-2 space-y-5", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-3xl bg-card border border-border p-6 shadow-soft", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { ref: mapContainerRef, className: "mt-4 h-72 md:h-96 rounded-2xl border border-border overflow-hidden relative", style: {
            ...
            !leafletLoaded && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute inset-0 flex items-center justify-center bg-muted text-xs text-muted-foreground", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "inline-flex items-center gap-1.5", children: [
